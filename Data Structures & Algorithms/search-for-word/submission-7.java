class Solution {
    ...
    public boolean exist(char[][] board, boolean[][] visited, int i, int j, String word, String s) {
        ...
        } else if (i < 0 || i >= board.length || j < 0 || j >= board[i].length || visited[i][j]
            || s.length() > word.length() || !word.startsWith(s)) {
            return false;
        }
        String n = s + board[i][j];
        visited[i][j] = true;
        boolean t = exist(board, visited, i + 1, j, word, n)
            || exist(board, visited, i - 1, j, word, n) || exist(board, visited, i, j + 1, word, n)
